...
const taskPath = path.join(import.meta.dir, "..", "tasks");
const inputPath = path.join(import.meta.dir, "..", "inputs");
...
if (day < 1 || day > 25) throw new Error(`day=${process.argv[2]} is not a valid day (must be1-25)`);
...
if (AOC_SESSION) {
	...
	const taskContent = 'import { input } from "../src/inputManager";\n';
	fs.writeFile(path.join(taskPath, `${day}-1.ts`), taskContent);
	fs.writeFile(path.join(taskPath, `${day}-2.ts`), taskContent);
}
